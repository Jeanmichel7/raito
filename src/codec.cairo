...
const MAX_BLOCK_WEIGHT: usize = 4_000_000_u32;
const MAX_BLOCK_SIZE: usize = 1_000_000_u32;
...
pub trait Encode<T> {
    ...
    /// Returns the weight of the encoded structure.
    fn encode_to(self: @T, ref dest: ByteArray, witness: bool) -> usize;

    /// Encode using Bitcoin codec
    /// Specify witness flag for including segwit fields (for wtxid calculation).
    /// Returns tuple of encoded byte array and weight of the encoded structure.
    fn encode(
        self: @T, witness: bool
    ) -> (
        ByteArray, u32
        ...
        let weight: u32 = Self::encode_to(self, ref dest, witness);
        (dest, weight)
    }
}

pub impl EncodeSpan<T, +Encode<T>> of Encode<Span<T>> {
    fn encode_to(self: @Span<T>, ref dest: ByteArray, witness: bool) -> usize {
        let mut wu: u32 = 0;
        let items = *self;
        wu += encode_compact_size(items.len(), ref dest);
        for item in items {
            wu += item.encode_to(ref dest, witness);
        };
        wu
    }
}
...
pub impl EncodeByteArray of Encode<ByteArray> {
    fn encode_to(self: @ByteArray, ref dest: ByteArray, witness: bool) -> usize {
        let len: usize = self.len();
        let wu: u32 = encode_compact_size(len, ref dest);
        dest.append(self);
        len + wu
    }
}
...
pub impl EncodeU32 of Encode<u32> {
    fn encode_to(self: @u32, ref dest: ByteArray, witness: bool) -> usize {
...
pub impl EncodeU64 of Encode<u64> {
    fn encode_to(self: @u64, ref dest: ByteArray, witness: bool) -> usize {
...
pub impl EncodeHash of Encode<Hash> {
    fn encode_to(self: @Hash, ref dest: ByteArray, witness: bool) -> usize {
...
pub impl EncodeTxIn of Encode<TxIn> {
    fn encode_to(self: @TxIn, ref dest: ByteArray, witness: bool) -> usize {
        let mut wu: u32 = 0;
        wu += self.previous_output.encode_to(ref dest, false);
        wu += (*self.script).encode_to(ref dest, false);
        wu += self.sequence.encode_to(ref dest, false);
        wu
    }
}

pub impl EncodeTxOut of Encode<TxOut> {
    fn encode_to(self: @TxOut, ref dest: ByteArray, witness: bool) -> usize {
        let mut wu: u32 = 0;
        wu += self.value.encode_to(ref dest, false);
        wu += (*self.pk_script).encode_to(ref dest, false);
        wu
    }
}

pub impl EncodeOutpoint of Encode<OutPoint> {
    fn encode_to(self: @OutPoint, ref dest: ByteArray, witness: bool) -> usize {
        let mut wu: u32 = 0;
        wu += self.txid.encode_to(ref dest, false);
        wu += self.vout.encode_to(ref dest, false);
        wu
    }
}

pub impl EncodeTransaction of Encode<Transaction> {
    fn encode_to(self: @Transaction, ref dest: ByteArray, witness: bool) -> usize {
        let mut wu: u32 = 0;
        wu += self.version.encode_to(ref dest, false) * SCALE_FACTOR;
        ...
        wu += self.inputs.encode_to(ref dest, false) * SCALE_FACTOR;
        wu += self.outputs.encode_to(ref dest, false) * SCALE_FACTOR;

        if witness {
            // Encode array of witnesses for each tx input
            for txin in *self
                .inputs {
                    wu += txin.witness.encode_to(ref dest, true);
                    println!("witness txin: {}", wu);
                };
            println!("witness: {}", wu);
        }

        wu += self.lock_time.encode_to(ref dest, false) * SCALE_FACTOR;
        wu
    }
}
...
mod tests {
    ...
    fn test_encode_txout() {
        ...
        let (bytes, _) = txout.encode(false);
    ...
    fn test_encode_outpoint() {
        ...
        let (bytes, _) = outpoint.encode(false);
    ...
    fn test_encode_tx1() {
        ...
        let (tx_encoded, tx_weight) = tx.encode(false);
    ...
    fn test_encode_tx_many_outputs() {
        ...
        let (tx_encoded, tx_weigth) = tx.encode(false);
        let tx_expected = from_hex(
    ...
    fn test_encode_tx_witness1() {
        ...
        assert_eq!(tx_weight, 573_u32);

        let (wtx_encoded, wtx_weight) = tx.encode(true);
    ...
    fn test_encode_tx_witness2() {
        ...
        let (tx_encoded, _) = tx.encode(false);
        let tx_expected = from_hex(
        ...
        let (wtx_encoded, _) = tx.encode(true);
        let wtx_expected = from_hex(
